...
| ID | 무엇을 본다 | FAIL 조건 |
|---|---|---|
| **D1** TOKENS-COPY | 정본 5파일(tokens · components · public · fonts · behaviors) 사본 | 정본과 내용이 다르다 (LF 정규화 후 대조) |
| **D2** PALETTE | 소스의 색 리터럴 | 허용집합 밖 · 폐기값(`#256ef4`·`#246BEB`) |
| **D3** SHELL | 로드 순서 · 프로필별 셸 · 파비콘 | 토큰이 자기 스타일보다 늦게 로드된다 · 대민 화면에 업무 GNB/LNB |
| **D4** FONT | 외부 폰트 CDN | jsDelivr·Google Fonts·unpkg·cdnjs 참조 |
| **D5** A11Y | 포커스 · 대비 | `:focus-visible` 부재 · 대체 없는 `outline:none` · (`enforce` 면) AA 미달 |
| **D6** CANON | 정본 자체 | 계약↔파일 불일치 · 토큰 AA 미달 · 프로필 블록에 치수 외 토큰 · 인벤토리↔CSS 불일치 · 생성물(갤러리 합본·DESIGN.md·레지스트리) 낡음 |
| **D7** REGISTRY | Tier 2 소비 프로젝트(`components.json` 있음) | 토큰 `@import` 없음 · `:root` 매핑에 `var(--ggc-*)` 아닌 값 · (WARN) `.dark` 팔레트 |
...
**사유 없는 예외는 불가능하다.** 등록된 예외 전부를 `<서비스>/docs/design-decisions.md` 의 "예외" 절에 모아
총량이 보이게 한다. 예외는 늘어나기 마련이고, **보이지 않는 예외가 드리프트다.**

## 증거 3종

`<서비스>/docs/design-evidence/` 에 파일로 남긴다 — `check_design.txt` · `1440x900.png` · `390x844.png` · `console.txt`.

1. **검사기 출력 전문** — `--gate` 의 stdout. 요약 줄만 붙이지 않는다
2. **스크린샷** — 1440×900 · 390×844 (확폭 채택 시 1920×1080 추가 — 계약 §8-5)
3. **브라우저 콘솔 0건** — 페이지 로드부터 주요 동선까지.
...
| D7 FAIL | 토큰 `@import` 가 빠졌거나 `:root` 에 값을 적었다 | `@import "../styles/ggc-tokens.css"` 를 첫 줄에, 매핑은 `@ggc/ggc-theme` 로 (`registry.md`) |

## 프런트가 없는 서비스

봇·배치는 **해당 없음**이다. 그렇게 `design-decisions.md` 에 기록한다.
