...
def build():
    ...
    slots = {
        ...
        "@@EXAMPLES_CSS@@": read(os.path.join(HERE, "examples.css")).replace(
            '@import url("gallery-fonts.css");',
            read(os.path.join(DESIGN, "ggc-fonts.css")).replace(
                './PretendardGOVVariable.subset.woff2',
                data_uri(os.path.join(DESIGN, "fonts", "PretendardGOVVariable.subset.woff2"), 'font/woff2'))),
...
def main():
    ...
    # Canonical font declaration with only the gallery-relative asset URL adapted.
    font_path = os.path.join(HERE, 'gallery-fonts.css')
    font_css = '/* Generated by build-standalone.py. Do not edit. */\n' + read(os.path.join(DESIGN, 'ggc-fonts.css')).replace(
        './PretendardGOVVariable.subset.woff2', '../fonts/PretendardGOVVariable.subset.woff2')
    if not os.path.exists(font_path) or read(font_path) != font_css:
        if check:
            print('FAIL  gallery-fonts.css is stale')
            return 1
        io.open(font_path, 'w', encoding='utf-8', newline='\n').write(font_css)
